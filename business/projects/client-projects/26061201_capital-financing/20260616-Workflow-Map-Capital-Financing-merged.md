...
**File storage — a live tension with the company-wide SharePoint push.** Danielle and Yasmine both use **Dropbox** as their working file-storage tool and have for years; Danielle described SharePoint as "cumbersome" and reported real co-editing sync conflicts (two people opening the same file simultaneously creates a duplicate copy) as a specific, experienced problem, not just unfamiliarity. Sensitive documents (e.g., the bank borrowing-base spreadsheets) currently reach Howie **only via direct email** — he has no shared-drive access to them at all. Danielle is open to changing this once a company-wide file-system decision is made, and separately raised that Segway's own document model links out to Dropbox-hosted files rather than storing them natively, which she sees as a possible argument for keeping Dropbox in the mix rather than a full SharePoint-only move. **This is directly relevant to Cross-Workflow Observations item 6 (SharePoint rollout) below** — real, working-preference resistance from two of the three leadership-team members, not just an adoption/training gap.
...
6. **New institutional knowledge infrastructure.** CEO is implementing SharePoint as "institutional knowledge bank" for all processes and documentation. All team members are being trained on SharePoint structure and usage, with enforcement to come. Clear benefit: distributed knowledge capture reduces tribal knowledge dependency on individual staffers (e.g., Christy's free-form Mighty approach). Risk: SharePoint adoption has historically been weak at Capital Financing ("It's not very user-friendly... staff can't find it"), so training and ongoing discipline will be critical to success. This is the first layer of a broader accountability framework (see #7). **Update (Danielle 1:1, 2026-09-XX):** two of the three leadership-team members (Danielle and Yasmine) have a genuine, experience-based preference for Dropbox over SharePoint — not unfamiliarity, but specific complaints about SharePoint co-editing sync conflicts, plus a possible technical argument (Segway's document model links out to Dropbox-hosted files rather than storing natively). Worth treating the file-system decision as still genuinely open rather than assuming SharePoint is the settled answer — see the full detail in the new **Controller & Finance Operations** section above.
...
- Company-wide file-storage decision (Dropbox vs. SharePoint vs. something else) is still genuinely open, not settled in SharePoint's favor — see Cross-Workflow Observations item 6.
