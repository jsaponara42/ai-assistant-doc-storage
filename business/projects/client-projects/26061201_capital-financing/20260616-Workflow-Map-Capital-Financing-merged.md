...
Referral Received *(Intake team / phone, email, text, web application, sister-company portal)* ⚠ not structured CRM data — confirmed multi-channel (2026-09-10 Christy call): law-firm calls/emails, broker/salesperson calls/emails, client calls/emails/texts, online application, or sister-company portal for connected attorneys; returning clients simply text
→ Intake Collected *(Rayna/Leifert / 3CX phone (unrecorded) + Mighty+SF)* ⚠ manual template, no validation; ⚠ volume spikes break this step; confirmed heavily scripted, branching client interview — see Core Step 2 for full detail
...
| **Controller / Finance Operations** | Danielle | W6 Steps 4–5 (reconciliation + close-out); company-wide finance functions not on any workflow spine — see new section below |
| **Website / SEO** | KJ Yeh | Not on a workflow spine — see new **Marketing & Sales Operations** section below |
...
- **Julius — Inactive Account Follow-Up (new, from Howie dictation 2026-07-28):** Julius handles follow-up with all inactive accounts (both case financing and pre-settlement) behind the scenes, sending templated emails and calls. He has **not had much success with responses**. He tends to fall **months behind** in his cadence, which defeats the consistency that follow-up requires. The CEO explicitly stated: "This is something I believe can be automated as well, does not need a manual person to be sending out templated emails. And actually, if we had it set up from an automation perspective, it probably would be more creative, more effective, and more consistent, where he seems to fall months behind in his cadences, which defeats the consistency that we're looking for on follow-up emails for whatever the purpose is we're looking for." **This is a high-priority automation candidate** — flagged as ranked want #10 in `xx_howies_wants.md`. Should be mapped as its own Process Trunk: **Inactive Account Follow-Up** (Julius, automated replacement TBD). Currently manual, low-success, cadence-defeated. **Update (2026-08-06 call):** Kaz has explicitly recommended cutting Julius outright rather than repositioning him as a VA — Howie is paying for output that isn't landing results, and Kaz's view is to automate the function and let the role go. Decision still pending with Howie. **Update (Howie's CMO overview document, 2026-08-30): the decision is no longer pending — Howie states directly he wants to move this to AI-assisted, better-automated generation and sequencing.** The scope is also now fully enumerated rather than treated as one blob: six distinct email motions run through Julius today — prospecting (by product line), reactivation (the 60-day inactive cadence), active-account nurture (confirmed as **not currently existing at all** — "any law firm that is active we should be nurturing with specific emails" is an explicit, unaddressed gap), thank-you emails after every new referral (Howie's own words: "should be automated"), post-conference follow-up (a 3-email cadence with little success, rebuild stalled in the Salesforce-to-MailChimp migration), and the long-running 15–19 touch law firm drip series (also mid-migration to MailChimp, Kaz-owned). See want #11 in `xx_howies_wants.md` for the full breakdown, and the new **Marketing & Sales Operations** section below for the rest of this document's findings.
...
- **Danielle** — Part owner, professional, and territorial about her role. Per CEO: "Danielle's a professional. Danielle's a part owner. Danielle's going to be very territorial." However, she supports bringing in external expertise for process improvement and is encouraging Michael (Injury Specialists client) to engage the same advisor. She has made independent management decisions (e.g., promoting a team member) without consulting Christy. **Update (2026-08-06 call):** Howie describes her as personally close ("family to me") and professionally high-level (prior SAP experience). No obvious automation surface in her own function per Howie, but she wants visibility into what's going on across the business. One of the three leadership-team members (with Christy and Yasmin) who told Howie directly they would not continue working with Josh. **Update (Danielle 1:1, 2026-09-XX): her own function turns out to have real automation-adjacent detail Howie didn't surface** — see the new **Controller & Finance Operations** section below for the full breakdown (post-settlement close-out mechanics, monthly bank reporting, payroll/401k/insurance/HR/state-registration administration, cash management). Self-describes as "more of a controller" doing tactical/recurring accounting work, distinct from Mark Kanye, the company's actual CFO, who owns bank relationships and financing strategy. Background: 12 years at SAP doing enterprise implementation work — she independently and unprompted validated JC's discovery-first methodology on this call ("design is... the most important and most time-consuming piece... a lot of times when you sit down with super users, they know what they do but they really haven't thought about how to make things better"), a notably more receptive read of the engagement's pace than Howie's.
...
**Confirmed intake-team headcount and systems (Christy 1:1, 2026-09-10):**
- **Headcount, as of this call:** one in-office employee (**let go the week of this call** — `[TO CONFIRM: which named individual — Rayna, Alejandro, or "Leifert"/Leaford — this was, and whether it affects the Tier 1 underwriting-training plan]`), one remote U.S.-based employee, and three remote employees based in Colombia. **Everyone on the intake team is bilingual (English/Spanish)** — confirmed business need given the volume of Spanish-speaking PI clients.
- **Phone system:** 3CX phone app — confirmed **calls are not recorded**; intake types notes in real time while the client is speaking.
- **Systems used, confirmed list:** Mighty (application system) and Salesforce in parallel — since the two don't sync, any attorney built in Mighty must be separately built in Salesforce so marketing materials reach them, meaning intake does duplicate data entry as standard practice (corroborates the IPR double-entry finding from the Kaz call). Also: **Funding Exchange** (checks other funding companies for prior liens on a case), and **TLO** for background checks — confirmed **TLO access is restricted to U.S.-based employees only**, so the three Colombia-based staff cannot run these checks themselves. Intake also does manual internet/social-media searches for mugshots and news coverage, since local-news Facebook posts of an arrest often appear well before any formal record does.
...
- **In:** Inbound referral or contact, confirmed multi-channel (Christy 1:1, 2026-09-10): (1) the law firm calls or emails directly, (2) a broker/salesperson calls or emails, (3) the client themselves calls, emails, or fills out an online application on the website, (4) a connected attorney submits through the sister company's portal, or (5) a returning client simply texts (once they have the company's text number) to request another advance.
- **Out:** New plaintiff funding opportunity, opened as an application in Mighty. `[TO CONFIRM: Salesforce record created now, or email-only until later?]`
- **Owner/System:** Intake team / phone (3CX, not recorded), email, text, or web form into Mighty.
- **⚠** Intake runs on emailed templates, not CRM forms — referral may never become structured CRM data at entry.
- **Confirmed client-contact pattern:** intake deliberately **texts the client first** rather than cold-calling, and lets the client call back on their own schedule — Christy's stated reason: clients cold-called mid-doctor's-visit either can't talk or assume the call is spam and ignore it.

### Core Step 2 — Intake Collected
- **In:** Plaintiff + case details, gathered via a live, heavily scripted phone conversation with the client. `[TO CONFIRM: full field list — partially confirmed below]`
- **Out:** Completed intake packet in Mighty (duplicated into Salesforce for the attorney/firm record).
- **Owner/System:** Rayna / Leifert (intake team) handle the core interview and packet; the two VAs handle downstream document collection (see **Shared Sub-Layer — Intake Support, Sub-Step A**) / 3CX phone (not recorded) + Mighty + Salesforce.
...
- **Confirmed interview structure (Christy 1:1, 2026-09-10):** Intake never asks an open-ended "tell me what happened" — clients are described as vague, and Christy estimates **~90% of the information intake needs is actively mined from the client** through a long, branching set of scripted questions covering the application and case-facts. The specific question set depends on case type, and **branches further within a type** — e.g., a motor-vehicle case has distinct question sets for rear-end, T-bone, sideswipe, and hit-and-run, and the next question asked often depends on the client's prior answer. Christy has described this as effectively a **flowchart/decision-tree structure** ("is this a motor vehicle accident? then ask these questions, and it branches from there") — this is the clearest articulation yet of what a structured intake-question tool would need to encode, and directly informs any future Ask-Howie/AI-assisted intake design (see Cross-Workflow Observations, new item on "Ask Howie AI").
- **Confirmed downstream research, new case (Christy 1:1, 2026-09-10):** for a new client, intake also pulls ID photos, prior-funding payoff status (via **Funding Exchange**, cross-checking other funding companies), child-support and bankruptcy records, and runs a background check via **TLO** (U.S.-based staff only — the three Colombia-based intake staff cannot access it) plus manual internet/social-media search for mugshots or news coverage, which often surface before formal records do.
- **Confirmed law-firm verification step:** intake separately calls the law firm to verify the attorney and paralegals are correctly on file, and gathers firm-provided basics — policy limits, whether liability is accepted or disputed, and prior funding status — which also vary by case type.
- **Confirmed training-documentation gap:** none of this is currently written up as a formal SOP. Christy and Yasmine built these processes organically over years (company was, in Christy's words, "a little bit of a shit show" when she joined) and never fully documented them in writing because the rules kept changing and rewriting training docs each time felt not worth the upkeep cost. Training has historically been shadow-training plus ad hoc summary emails after training calls; Christy is mid-way through moving those emails into a SharePoint area ("Team Christy") but confirms it is **not complete and not authoritative yet** — do not treat SharePoint intake content as final. Christy pushed back explicitly on Howie's requested short timeline for a full intake SOP, given ~20 years of accumulated judgment behind it; framed a Claude-assisted pass at organizing her existing training emails into categorized, flowchart-style question trees as a good, realistic first step rather than a from-scratch SOP write-up.
...
- **Confirmed intake→underwriting hand-off logic (Christy 1:1, 2026-09-10):** intake can only kill a case **without** escalating to Christy in two situations — the attorney of record declines to fund it, or the client cancels outright. Every other outcome routes to Christy: either she reviews the submitted application/summary herself, or intake calls her directly for a same-call read (e.g., an abusive or threatening client, or a client who already has advances with three other funding companies). She estimates she sees or hears about essentially every case at some point.
- **Confirmed rules-based vs. judgment ("art") split, and why it matters for automation:** Christy is explicit that her underwriting decision has two layers — a **rules-based layer** (state law, case type, liability clarity, minor-funding eligibility, policy limits) that changes with case law and is currently held entirely in her head, and a **judgment/"art" layer** (gut read on the attorney's reputation and staff quality, website credibility, how long they've practiced, whether a case is being oversold) built from ~20 years in the industry. She was direct that automating away the judgment layer "just won't work" and isn't the goal — the stated goal (agreed by both Christy and JC on the call) is to structure and encode the rules-based layer as much as possible so Christy spends more of her time on the judgment calls only she can make, not on checklist-style eligibility screening anyone could run.
- **⚠ Confirmed volatility problem, not previously documented:** the rules-based layer itself changes on short notice with case law (e.g., a state flipping from pure comparative to modified comparative fault instantly rules out certain motor-vehicle case types) or with Howie's own ad hoc decisions (e.g., deciding to stop funding in a state, or personally approving an exception for an attorney he met at dinner). **No defined mechanism currently exists for Christy to push a rule change out to the intake team when this happens** — this is a concrete, scoped automation/tooling gap worth carrying into the opportunity map, distinct from the underwriting-authority issue already flagged in Cross-Workflow Observations item 1.
- **Confirmed denial-communication ownership:** Christy personally handles all outbound communication to the law firm on a denial or adverse decision, rather than delegating it, because a poorly worded message risks losing the firm's business permanently — this is a deliberate risk-management choice, not a training gap.
- **Segway note:** Christy expects Segway to provide an automated eligibility checklist/kill-recommendation when a file reaches underwriting, but expects to still override it in some cases (e.g., a new law-firm relationship Capital Financing wants to build even where the checklist would recommend a kill) — consistent with the rules-vs-art framing above.
...
- **Confirmed dual-entry control (Danielle 1:1, 2026-09-XX):** this is a deliberate check-and-balance, not redundant busywork. Yasmine receives the physical/incoming check and assigns it to the correct case **inside QuickBooks** first. Danielle then independently applies the same payment to the advance **inside Mighty**, which is her own cross-check that the case-level amount matches what was recorded on the financial side — effectively two people, two systems, confirming the same payment before it's treated as settled. **Confirmed join key:** Mighty and QuickBooks are reconciled by the **client/plaintiff's name**, not a shared case ID — QuickBooks has no field for Mighty's case/lien ID, and a single plaintiff can carry multiple liens (multiple advances against one case, e.g. a motor-vehicle claim plus a workers'-comp claim) which the name-based match has to disambiguate using amount and date rather than an ID.
- **Confirmed policy, one-payment-per-settlement:** Capital Financing requires attorneys to send one payment per settlement, never a combined check covering multiple cases — specifically so a payment can always be traced back to an exact amount in the original contract during a financial audit.
...
- **Confirmed mechanics (Danielle 1:1, 2026-09-XX):** in Mighty, closing means applying the payment to the advance and then removing herself as the advance's **"primary owner"** — that removal is what flips the record's status to closed/full payment. She does this only after independently confirming in QuickBooks that the cash actually hit the bank account, not merely that Mighty shows a payment logged — a deliberate cash-vs-recorded-payment check, distinct from the case-matching reconciliation in Step 4.
- **Confirmed monthly close, not previously mapped as its own process:** Danielle runs a full **monthly financial close** — for each month, she confirms every settlement has been applied, every deposit has hit the bank, then runs a report from Mighty and the equivalent report from QuickBooks for the same month and confirms the two totals tie out exactly. Small discrepancies are common and explainable (e.g., a ~$25 wire fee someone forgot to enter, or a case with two liens where only one needed to be paid off) and get corrected as part of the close. This close typically takes about a week each month.
- **Confirmed bad-debt / write-off handling, not previously mapped:** when Yasmine, Howie, and Christy jointly determine an advance is likely uncollectible — most commonly because the underlying case was lost, and company policy is that a plaintiff owes nothing back if their case loses — the advance is tagged **"unassigned"** in Mighty (21 advances carried this tag as of this call). Danielle reviews these periodically; some get written off immediately, others are held in case of a later recovery. By Q3/Q4 of a given year she's typically resolved most of the year's unassigned advances — marking them non-payment in Mighty and writing them off as uncollected in QuickBooks.

---

# Controller & Finance Operations (Danielle's Domain)

**Confirmed via Danielle's 1:1 discovery call, 2026-09-XX.** This is not a workflow spine — it's a company-wide finance/back-office function that supports every workflow above without being triggered by any single case. Danielle self-describes as "more of a controller" doing tactical, recurring accounting work; the actual CFO (bank relationships, financing strategy) is **Mark Kanye** `[TO CONFIRM: exact name/spelling — a separate "Mark Gagne" is also named later on this same call regarding quarterly tax distributions; confirm whether this is the same person differently transcribed, or two different advisors]`. Included here because several pieces overlap directly with Workflow 6 (bank reporting depends on the same Mighty/QuickBooks data Danielle reconciles case-by-case) and because it surfaces automation-adjacent detail Howie's own account of her role didn't capture.

**Monthly bank reporting — Borrowing Base Certificate.** Danielle prepares a monthly report in the lender bank's own template (an Excel spreadsheet), listing every individual outstanding advance on the balance sheet (~$9.6M outstanding as of this call) since the bank requires line-item detail, not just a total. Each advance line ties to a specific lien via a bank-assigned code, date, and amount. The company can borrow up to **85%** of the total outstanding-advance balance under its line-of-credit agreement; utilization as of this call was ~$5.72M. **Confirmed: this report is generated directly from QuickBooks and takes about three minutes to produce once the monthly close (Workflow 6, Core Step 5) is finished** — already highly automated on Danielle's own assessment, not a manual-effort target.

**Quarterly bank compliance certificate.** Separately from the monthly report, Danielle prepares a quarterly compliance certificate demonstrating the company is meeting its loan covenants: a **Fixed Charge Coverage Ratio** (minimum 1.5x; the company was at 1.57x as of this call) and a **Senior Funded Debt-to-EBITDA ratio** (must stay under 3.0x — this ratio is what limits how much can be distributed to the partners, since partners are paid via distributions rather than salary under the partnership structure). Both are calculated on a trailing-12-month basis. This takes longer than the monthly report because the covenant math itself is manual. The company has failed a covenant only once or twice historically; when that happens, they proactively tell the bank their remediation plan rather than waiting to be asked.

**Cash management.** Danielle manages cash across both entities and implemented a **ZBA (zero-balance account) sweep** tied to the line of credit: a master account sweeps to zero every night — if cash is needed, it automatically draws from the line of credit; if there's a surplus, it automatically pays the line down. In her own words, this "decreased my stress quite a bit" — flagged here as an already-solved problem, not an automation target.

**Accounts payable.** Danielle handles all company payables — vendor payments via credit card, ACH, and wire through the bank's portal. Distinct from Yasmine's team, who fund advances to plaintiffs/leads; Danielle's payables are the company's own operating expenses and vendor relationships.

**Payroll & benefits.** Runs biweekly payroll for both companies via **ADP**. Sales commissions are sent to Howie and Danielle; Howie reviews and approves for accuracy, then Danielle enters them into payroll — direct dependency on the commissions-spreadsheet accuracy problem already flagged elsewhere (want #7 in [[xx_howies_wants]]). Also administers 401(k) elections and a **Safe Harbor 401(k)**: the company contributes 3% of gross salary for every eligible employee (1+ year tenure) regardless of whether that employee personally contributes.

**Insurance.** Manages all company insurance policies: Directors & Officers, cyber, workers' comp, and property & casualty.

**HR (partial, being handed off).** Danielle handled HR historically despite explicitly not wanting to ("I'm not an HR person... I don't have so much of a liability" as a stated concern). An outsourced HR provider was brought on ~3 years ago and underperformed; **a new outsourced HR company was hired the same day as this call.**

**State & regulatory registrations (growing, not yet systematized).** Two distinct registration types, confirmed: (1) standard Secretary of State business registration (home state Georgia, plus others as the company expands), and (2) **litigation-funding-specific state registrations** required in certain states to operate as a funding company — confirmed active/in-progress in Utah, Kansas, Oklahoma (just completed), and New York (due December). Some states require both registration types, some only one. Danielle uses a registered-agent service (**CSC**) for this and sometimes has to obtain a bond as part of a state's requirements. All of these require **annual renewal**. She mentioned, unprompted, that she's "just started to maintain a spreadsheet because it's growing" — **a real, scaling administrative-tracking need with no system behind it yet**, worth flagging as a small but genuine opportunity (a tracked renewal calendar/reminder system) even though it wasn't raised as a complaint.

**Other recurring duties, confirmed as hers alone:** county-level business license management; vendor W-9 collection; year-end 1099 filing; quarterly estimated tax payments (coordinated with the outside tax advisor on partner distribution amounts needed to cover the quarter's tax liability).

**File storage — decided (SharePoint), but organization and adoption are the real gap.** Danielle and Yasmine both use **Dropbox** as their working file-storage tool and have for years; Danielle described SharePoint as "cumbersome" and reported real co-editing sync conflicts (two people opening the same file simultaneously creates a duplicate copy) as a specific, experienced problem, not just unfamiliarity. Sensitive documents (e.g., the bank borrowing-base spreadsheets) currently reach Howie **only via direct email** — he has no shared-drive access to them at all. **Decision (confirmed, final): SharePoint is the company-wide system.** The real gap isn't which tool — it's that the company's file organization and habits around it are poor across the board, and Dropbox-vs-SharePoint has been functioning as a proxy for that deeper problem. Danielle and Yasmine's Dropbox preference is a change-management challenge to work through during rollout, not a reason to run two systems. **This is directly relevant to Cross-Workflow Observations item 6 (SharePoint rollout) below** — real change-management work needed with two of three leadership-team members, not just a tool-selection question.

**Tooling decision, this call:** Danielle does not have a Capital Financing email address, so she was not auto-enrolled on the company's Claude team seat. Given her workflows are QuickBooks/ADP-driven and already heavily automated on her own assessment, and she's already comfortable with ChatGPT personally, JC decided not to onboard her onto Claude specifically — she can keep using ChatGPT, with the informal ask that she not actively promote it over Claude to the rest of the team, to avoid undermining the single-tool standard being set for everyone else.

---

# Marketing & Sales Operations

**Confirmed via a company/marketing overview document Howie wrote himself for an incoming CMO hire, dated 2026-08-30, added to the vault 2026-09-11.** Like the Controller section above, this isn't a workflow spine — it's the marketing and sales-enablement layer that feeds Workflows 3, 4, and 5. Included in full because it's the richest single source yet on the sales side, and because Howie's own document is explicit that sales is the current top priority for this engagement (see the client-facing Opportunity Map for how that reprioritization was applied). Much of this document is CMO-scope strategic and creative work (website redesign, content strategy, video), not automation — flagged inline where that's the case, since it's context worth having even though it isn't something to build here.

**Business model, confirmed and more precise than previously documented.** Capital Financing is deliberately **law-firm-first, not consumer-facing** — a positioning choice, not an accident. The company does not want to compete with PE-backed firms that market pre-settlement funding directly to plaintiffs; instead, marketing dollars go toward law-firm-facing trust and awareness (conferences, referral relationships), and Financial Consultants exist specifically to build and work those law-firm relationships. Two product lines: **Pre-Settlement Funding** (advances against a pending case, sourced almost entirely through referring law firms, occasionally through organic internet search) and **Case Expense Financing** (financing litigation costs directly for the firm's benefit — named explicitly as the product Howie most wants to grow).

**New thread, not previously documented anywhere in this project: ancillary broker relationships.** Howie has personal relationships with outside financial institutions that can offer law firms lines of credit and other financing Capital Financing doesn't itself provide, including post-settlement financing. This isn't marketed anywhere today, on the website or otherwise. Howie is weighing a separate entity to market it versus folding it into the existing site. This is CMO-scope strategic work, not an automation opportunity, but worth tracking since it could eventually generate its own referral workflow.

**Team roles, confirmed and consolidated:**
- **KJ Yeh** — Website & SEO Manager, long-tenured, owns the site and SEO exclusively (no PPC, no consumer ad spend). **Confirmed accountability gap:** Howie states directly that KJ's monthly reports are "like Chinese" to him, meetings are rare, and he has no independent way to verify KJ's work is actually moving rankings, traffic, or conversions, despite KJ's long tenure. Howie's own words: "as we know people get lazy after time." Now tracked as its own opportunity — see [[xx_howies_wants]], want #26.
- **Julius** — confirmed detail now folded into the Process Trunks note above (six email motions, LinkedIn network growth) and want #11.
- **Financial Consultants** — Capital Financing's own term for its sales team (not "BDRs"). Own law-firm relationship management and referral generation. No dedicated sales manager exists; Howie personally absorbs that role in the interim (see Sales Performance & Accountability Framework above).
- **Kaz** — confirmed as also owning the Mailchimp migration and conference attendee-level reporting/ROI tracking build, in addition to everything already documented in the Salesforce Platform Reference section above.

**Channel snapshot, confirmed:**
- **Website/SEO:** built for plaintiffs researching pre-settlement funding online, not for law firms, and doesn't mention Case Expense Financing or the ancillary broker relationships at all. Redesign/positioning work is CMO-scope, not automation.
- **Email — six motions:** see the Julius process-trunk note above and want #11. The active-account nurture gap (no cadence exists at all today) and the thank-you-email automation are the two most concrete, immediately actionable pieces inside this motion set.
- **Social media:** Instagram, Facebook, LinkedIn, run by Julius, considered low-impact for lead generation, retained mainly for branding. Howie wants a content approach built around his own industry reputation and 16 years of company tenure, including video — CMO-scope creative work, not automation.
- **Conferences:** 15–20 a year at real expense. **Confirmed, new:** ROI tracking connecting attendance to eventual referrals is named as a top priority, distinct from the territory/assignment question already tracked in want #8. Kaz is already building attendee-level Salesforce reporting as the foundation for this — now tracked as its own opportunity, want #25.
- **Intake:** confirmed as predominantly warm, law-firm-referred leads rather than cold consumer inquiries — corroborates the existing intake documentation in Workflow 1/2 and the Shared Sub-Layer above; no new findings here beyond what Christy's 1:1 already covered.
...
6. **New institutional knowledge infrastructure.** CEO is implementing SharePoint as "institutional knowledge bank" for all processes and documentation. All team members are being trained on SharePoint structure and usage, with enforcement to come. Clear benefit: distributed knowledge capture reduces tribal knowledge dependency on individual staffers (e.g., Christy's free-form Mighty approach). Risk: SharePoint adoption has historically been weak at Capital Financing ("It's not very user-friendly... staff can't find it"), so training and ongoing discipline will be critical to success. This is the first layer of a broader accountability framework (see #7). **Decision confirmed, final (per JC, following Danielle's 1:1): SharePoint is the company-wide file system.** Danielle and Yasmine's genuine Dropbox preference (co-editing sync conflicts, years of habit) is real and needs deliberate change management during rollout, not a reason to revisit the tool choice. **The actual gap is organizational, not technological:** the company's file taxonomy and structure are weak across the board ("a hot mess," per Christy's independent characterization), and a real information-architecture pass — folder taxonomy, naming conventions, governance rules, migration off Dropbox and out of one-off email attachments — is needed before SharePoint can function as the institutional knowledge bank it's meant to be. See the new opportunity on the Opportunity Map deliverable.
...
8. **"Ask Howie AI" concept, new (Christy 1:1, 2026-09-10).** Christy independently raised the same idea Howie has voiced separately: an internal AI FAQ tool ("Ask Howie") that the sales team and staff could query directly for standard questions (e.g., PTO process, intake case-type questions) instead of pinging Howie or Christy repeatedly, or sorting through SharePoint. Her framing: SharePoint is currently "a hot mess" and over-documenting causes staff to disengage entirely ("people don't look at documents when they get over-documented"); she sees SharePoint's proper role as the backing library of source-of-truth documents (e.g., a specific handbook clause), with an AI layer summarizing and answering on top rather than routing employees to browse folders directly. **This is not yet logged as a ranked want in [[xx_howies_wants]]** — worth adding as its own item and cross-referencing against want #20 (Howie's personal inbox AI assistant), which is adjacent but distinct (personal inbox triage vs. a staff-facing FAQ tool).
9. **Post-Josh transition — reprioritization and new working model (2026-08-06 call).** Howie independently confirmed, in his own words, that operations was not where this engagement's help was most needed ("we don't have a lot of issues operationally... Christy and her leadership team are doing great") — sales productivity and tech/automation efficiency are the actual priority, validating PO-005 over the operational/COO-style work Josh focused on. Howie also self-attributed part of the operational gap to his own lack of delegation — Christy holds the DOO title but was never clearly directed to lead SOP/workflow rollout ("I blame myself"). Going forward, JC works primarily through Christy for team dissemination rather than managing departments directly, and individual reset conversations with Christy, Danielle, and Yasmin are planned before further work, specifically to establish a different working relationship than the one the team had with Josh. See [[business/projects/client-projects/26061201_capital-financing/xx_Project_Learnings]] for full call detail and `client-facing-deliverables/20260806_proposal-capital-financing-ai-automation-mapping.md` for the resulting scoped proposal.
...
- **Current status, confirmed actioned (2026-09-XX call):** No longer receives a salary — CEO moved her to commission-only, with an increased commission rate to offset the change, and told her directly the choice to generate business or not is now hers. Company credit card status still pending review.
- **New confirmed example, Kansas territory (2026-09-XX call):** A Kansas law firm inquiry came in through Julius's outbound automation, not through Victoria's own outreach, and was routed to her since she's the assigned FC for that territory. She had been specifically directed to work Kansas accounts and, per this inbound lead's origin, had not been doing so. Cited as concrete evidence for the broader "wrong list, no targeting discipline" pattern raised in this same call — see the new **Slack-Based Sales Workflow** section below for the targeting/prioritization fix under discussion.
...
**Commission structure, confirmed — no structural excuse for prioritizing existing accounts over new business (2026-09-XX call).** Unlike larger companies, Capital Financing doesn't split FCs into "hunter" (new business) and "farmer" (account management) roles — one person does both, and the compensation structure is designed so that's always in the FC's own financial interest: commission continues in perpetuity on every future funding from an account they originally brought in (no de-incentivization on repeat loans, second/third/fourth advances, or year two of a relationship), and commission rates actually increase past a volume threshold rather than capping. This is CEO-stated context for why he reads underperformance as a motivation/effort problem rather than a structural one — worth holding as his stated framing, not necessarily the full picture, when discussing FC performance.

## Slack-Based Sales Workflow (designed 2026-09-XX, phased rollout)

**Design session with the Salesforce Administrator.** The target system: each morning, a consultant gets a Slack message split into three tiers — (1) **Opportunities needing attention**, meaning contacts who've already had a documented positive response (an Opportunity record only gets created after a positive contact, not before); (2) **high-priority net-new contacts**, the model example being recent conference attendees; (3) **lower-priority net-new contacts**, including inactive-account reactivation candidates. Clicking a name from the Slack message opens the record directly; logging a call or email happens in Slack, one click, without opening Salesforce at all. At the end of each day, leadership gets an automated Slack DM report showing each FC's call, email, and in-person-visit counts.

**Follow-up mechanics, confirmed.** Each Opportunity stage carries a time-based reminder (e.g., 7 days, 14 days since last contact) that resurfaces it as a priority. If a contact hasn't responded within 30 days of the last touch, they cycle back into the ordinary outreach list rather than staying flagged indefinitely. Deciding when to stop pursuing an unresponsive contact (after how many attempts) is explicitly a per-FC judgment call, not a system rule — this is a training point, not something to automate away.

**No hard-coded volume requirement.** The system does not enforce how many people an FC must call or email in a day — that remains a management/coaching decision for Howie, not a Slack rule. The system only tracks and reports what actually happened.

**Conference list activation, confirmed mechanism, still manual.** Slack has no way to know when a conference has ended. The proposed fix: Kaz tags a conference's contact list "inactive" before and during the event, then manually flips it to "active" once the conference ends and post-conference follow-up should begin — that flip is what makes the list auto-populate into FCs' high-priority tier. This still requires a person (Kaz, on Howie's cue) to trigger it; it is not automatic end-to-end.

**Correction, phased rollout (same-day follow-up call, 2026-09-XX): the full system above is not what's launching in the near term.** Given essentially no FC is logging activity consistently today, building the full system now would mean building it on almost no real data. The actual immediate plan, in order:
- **Step 0:** Every FC bookmarks three Salesforce views — active accounts, inactive accounts, and prospects — so there is no ambiguity about where a day's contacts come from. To be verified live, screen-share style, one FC at a time if needed.
- **Step 1 (within ~2 weeks):** Every call and email gets logged as a Salesforce activity at the time it happens, for every contact, no exceptions. A one-page training document covers this alone. Still inside Salesforce, not Slack — Slack isn't live yet.
- **Step 1.5 (same ~2-week window):** The Salesforce Administrator adds one simple report: raw call and email volume per FC per day, including the CEO's own volume as a built-in benchmark. Estimated a couple of hours of build time — intentionally the simplest possible version, no segmentation by list type yet, since that needs consistent logging data first.
- **Deferred:** The full three-tier Slack system, the automated end-of-day report, and the conference-tagging automation remain the real target and the design above is final, but none of it starts building until Step 1's logging habit is real and holding. **An immediate, informal benchmark in the meantime:** FCs should be logging more calls and emails than the CEO logs himself.

**Monthly KPI targets, confirmed with a specific example (2026-09-XX call).** Howie wants a monthly automated report per FC showing dollar volume funded against a target (illustrative figure used: $75,000/month total, split roughly $50,000 pre-settlement and $25,000 case expense), funding counts per product line (e.g., 15 pre-settlement fundings/month as a target), and count of new referring accounts. Declines are tracked but explicitly **not** something to manage against directly — his framing: focus on generating enough opportunities and referrals, and the approval/decline ratio takes care of itself. This is all measurable today from IPR/application data already flowing through Mighty.
...
- **New (2026-09-10 Christy call):** confirm which named individual (Rayna, Alejandro, or "Leifert"/Leaford) was the in-office intake employee let go the week of this call, and whether that affects the Tier 1 underwriting-training plan for Rayna/Leaford. Reconcile against the confirmed current headcount (1 in-office [departing], 1 remote U.S., 3 remote Colombia, all bilingual).
...
- **New (2026-09-10 Christy call):** design a mechanism for Christy to push a rules-based underwriting change (state law shift, minor-funding eligibility, a state Howie decides to stop/resume funding in) out to the whole intake team — none exists today.
- **New (2026-09-10 Christy call):** confirm realistic timeline/scope for turning Christy's training emails into structured, flowchart-style intake question trees (by case type, e.g. motor-vehicle sub-types) — she was explicit a full formal SOP is not a same-week deliverable given ~20 years of accumulated, still-evolving judgment behind it.
...
**Controller & finance operations (new, from Danielle 1:1, 2026-09-XX)**
- Confirm whether "Mark Kanye" (CFO, bank relationships) and "Mark Gagne" (quarterly tax-distribution advisor) are the same person differently transcribed, or two separate advisors.
...
- Whether the ancillary broker-relationship marketing opportunity (post-settlement financing referrals) gets folded into the existing site or spun into a separate entity — Howie's own framing suggests this is undecided even on his end.
- Whether "click funnels," mentioned twice in the document without specifics, refers to a concrete tool/workflow Howie has in mind or is aspirational language worth clarifying directly before scoping anything against it.
