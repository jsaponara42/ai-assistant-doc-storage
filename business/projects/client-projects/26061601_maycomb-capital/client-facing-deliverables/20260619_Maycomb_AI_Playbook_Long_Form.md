...
9. **NEEDS CONFIRMATION with Ariella (July 7 call): Which Claude plan Maycomb is actually on, and how billing works.** Anthropic's standard Enterprise plan bills seats and token usage **separately** — the seat fee covers platform access only, and every token used in chat, Claude Code, or Cowork is metered at standard API rates on top of the seat cost, with no included usage allowance. This is materially different from a **Team** plan, where a usage allowance is bundled into the seat fee. Confirm which one Maycomb is on — it changes both the cost-control guidance the Playbook should give and whether Section 2/4's "use freely, sanity-check" framing needs a cost caveat added.
10. **NEW since original draft — Claude Fable needs to be addressed in Section 4 (model guidance).** Fable launched June 9, 2026 and is priced at $10/$50 per million input/output tokens — exactly double Opus's rate, making it the most expensive generally available Claude model. It's built for long-running, complex agentic tasks, not everyday drafting or chat. On Maycomb's likely usage-based billing (see item 9), routine use of Fable for normal tasks would burn tokens at a premium rate with no included allowance — the team would only discover the cost on the bill. Recommend adding an explicit line to Section 4: **almost no one at Maycomb should reach for Fable by default** — stick with Sonnet as the default, escalate to Opus for genuinely hard tasks, and treat Fable as a rare, deliberate choice for the AI owner to approve, not a self-serve option for routine work.
