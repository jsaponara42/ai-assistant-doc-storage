...
The whole thing fits on a few pages on purpose. If a rule here ever feels heavier than the work it's protecting, that's a bug — tell the AI owner (Section 9) and we'll fix it.

---

## Part One: What you do every day (the workflow)

The sections below walk through the forward-moving loop: take something annoying, run it through Claude, and if it works, make it the team's standard. This is the part you'll actually *do*.
...
1. **Add one gripe to the list.** The most annoying recurring task you can think of. That's the whole on-ramp. (Section 1)
2. **Try one thing with Claude.** Anything low-stakes — a draft, a summary, a question. Get a feel for it. (Section 6)
3. **If something works, say so.** Tell the AI owner or drop it in the "prompts that work" doc. That's how it spreads. (Section 5)
...
## 2. Prompt modules — write once, reuse everywhere
...
This is a different kind of reuse than a SKILL (Section 5). A SKILL captures *how to do a task*. A prompt module captures *what Claude should already know* before you even ask it to do anything.
...
**Ownership, raised on the July 14, 2026 call:** Martina suggested the centralized modules, especially "About Maycomb Capital," shouldn't have everyone writing their own version of how to describe the firm. She proposed an **AI steering committee** to own and approve the canonical versions, mirroring an existing IT steering committee Maycomb already runs. She specifically flagged Maycomb's newly hired comms person as a natural fit, since they're already rethinking how Maycomb is branded and described externally, and this is directly in that lane. Not yet decided whether this becomes a standing committee or just clear individual ownership; worth resolving before the modules above get published team-wide.
...
- **This pairs with the trust check in Section 6.** A high-stakes task isn't automatically an Opus task — "how much it matters if it's wrong" (do I verify and own it?) is a separate question from "how hard is it to get right" (which model). A high-stakes but straightforward task can run on Sonnet and still get carefully checked by you.
...
> **Gripe (1) → try it low-stakes (6) → it works → capture as a Phase 1 Project/prompt (5, Phase 1) → if it matters, package it as a SKILL (5, Phase 2).**

---

## Part Two: How to do it safely (the guardrails)

These sections cover the rules, the governance, and the ownership. You'll reference them when you need to know whether something is safe to send, who to loop in, or how to keep the system running.

---

## 6. The trust gate — what to ask before you trust an answer
...
**One separate rule, regardless of stakes:** when real Maycomb information is involved, use the **enterprise version of Claude only** — never a free or personal AI account (free ChatGPT, Claude, Gemini, etc.), since those can learn from what you type. Enterprise Claude is set up so your inputs aren't used to train the model, which is why it's safe for the real thing. (More in Section 7.)

---

## 7. Where data goes — which tools for which data

This section is about *where* real Maycomb information is allowed to go — a separate question from the "how much do I trust the output" gate in Section 6. The core principle is short:
...
## 8. The monthly review — a 30-minute look, once a month
...
## 9. Who runs this — the AI owner and enforcement
...
2. **The gripe list (1)** is the always-on input — nothing's too small, and it doubles as capturing how the work gets done.
3. **Prompt modules (2)** run alongside the gripe list as a second, separate habit — reusable background context (about you, your role, Maycomb, borrowers, investors) written once so it never has to be re-explained.
4. **Picking the model (3)** keeps work fast and costs down — default to Sonnet, Haiku for simple bulk, Opus for the genuinely hard.
5. **Tools that act (4)** keeps the higher-risk tools — Claude Code and Cowork — off by default until the AI owner approves a specific use case.
6. **From win to standard (5)** turns one person's win into the team's standard — light now (Projects + prompts doc), fuller later (packaged SKILLS).
7. **The trust gate (6)** is the core safety habit — low stakes, sanity-check and go; high stakes, you own the output before it's trusted or sent, and a new high-stakes use case gets a heads-up to the owner plus a proof run the first time.
8. **Where data goes (7)** keeps real-data decisions fast and consistent — to be completed once the stack is confirmed.
9. **The monthly review (8)** is where the list actually turns into decisions.
10. **Who runs this (9)** is the governance — the AI owner and enforcement authority.

---

## Appendix: Open items (for JC / Maycomb, not the team-facing version)

1. ~~Pick the lowest-friction home for the gripe list~~ — **RESOLVED: Slack** (confirmed by Danielle).
2. ~~Confirm Maycomb's full tool stack~~ — **RESOLVED:** Claude (Team plan — corrected July 17, 2026, was previously listed as Enterprise), Excel/loan servicing model, LeverPoint, Affinity (CRM), SharePoint, ShareFile, Slack, Teams (video only), DocuSign, Adobe, Expensify. Section 7 can now be built out.
...
13. **NEW, July 20, 2026 — Prompt modules concept added (JC).** New section inserted (now Section 2): reusable, write-once context blocks ("about me," "about my role," "about Maycomb Capital," "about our borrowers," "about our investors"). A first-draft "About Maycomb Capital" module is included, built from existing discovery documentation. Needs: (a) JC's sign-off on scope/framing before this goes to Maycomb, (b) Martina/deal-team review of the borrower and investor modules once drafted, since that content leans on Ariella's July 7 framing and she's off the account, (c) a decision on whether this gets its own numbered section (renumbering 2–7) or stays as-is for the working draft — currently unnumbered to avoid a disruptive renumber mid-review.
14. **NEW, July 14, 2026 — AI steering committee idea (Martina, live on the call).** Proposed owning the prompt-module content (see Section 2) through a governance body mirroring Maycomb's existing IT steering committee, with the new comms hire flagged as a natural fit given their branding work. Not yet decided whether this becomes a standing committee or informal ownership — needs a decision before the modules get published team-wide.
